...
class Visualizer():
    ...
    def save_custom_images(self, image_dir, visuals, image_path, webpage=None):        
        dirname = os.path.split(image_path[0])[0]
        image_dir = os.path.join(image_dir, dirname.strip('/'))
        util.mkdir(image_dir)
        name = os.path.basename(image_path[0])
        name = os.path.splitext(name)[0]        
