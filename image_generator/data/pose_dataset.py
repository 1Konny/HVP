...
class PoseDataset(BaseDataset):
            ...
            def pose_paths_match(dp_paths, dataroot):
                ...
                for batch_dir in dp_paths:
                    ...
                    for top_dir in batch_dir:
                        recording, color_mask_name = top_dir.strip('/').split('/')[-2:]
                        frame_name = color_mask_name.replace('_IUV', '')
                        frame_root = list(Path(dataroot).glob('*_img/%s' % recording))[0]
    ...
    def __getitem__(self, index):
        ...
        for i in frame_range:
            ...
            dp_paths.append(dp_path)
        
        if not self.opt.isTrain:
            self.A, self.B = A, B
            self.frame_idx += 1            
        change_seq = False if self.opt.isTrain else self.change_seq
        return_list = {'A': A, 'B': B, 'inst': 0, 'A_path': dp_path, 'A_paths': dp_paths, 'B_path': img_path, 'change_seq': change_seq}
    ...
    def get_image(self, A_path, size, params, input_type):
        ...
        elif input_type == 'densepose' and not self.opt.isTrain:
            if A_img.mode == 'L':
