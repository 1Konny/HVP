...
if [ "$DATASET" == "KITTI_64" ]; then
    CHANNELS=19
    DATA_ROOT='datasets/KITTI_64'
elif [ "$DATASET" == "KITTI_256" ]; then
    CHANNELS=19
    DATA_ROOT='datasets/KITTI_256'
fi
...
if [ "$MODE" == "ds" ]; then
    CMD="deepspeed --include=$GPU_FLAG inference.py --deepspeed --deepspeed_config deepspeed_util/ds_config_inference.json"
elif [ "$MODE" == "dp" ]; then
...
CMD="${CMD}\
    ...
    --n_past $N_PAST --n_future $N_FUTURE --n_eval $N_EVAL --frame_sampling_rate $FRAME_SAMPLING_RATE --n_prediction $N_PREDICTION \
    ...
    --name $NAME --last_frame_skip $LOAD_CKPT_TYPE"
echo $CMD
eval $CMD
